...
```
presence/
├── run.ps1                         Windows launcher (starts Postgres via Docker or local fallback, uv/bun sync, opens servers)
...
├── TODO.md                         Outstanding roadmap items (shipped work lives in git history)
├── scripts/
│   └── migrate_pickle_to_bytes.py  One-time SQLite pickle -> raw float32 embedding migration
├── backend/
│   ├── pyproject.toml              uv project config, pytest settings, and ruff/ty lint config
...
.\run.ps1
```
The script performs preflight checks, syncs backend dependencies (`uv sync`), installs frontend dependencies (`bun install`), provisions PostgreSQL, and opens separate terminal windows for backend and frontend.

Database provisioning is resilient:
- If Docker is running, it starts the container with `docker compose up db -d` (port `5432`).
- Otherwise it falls back to a local PostgreSQL installation, initializing a cluster in `.pgdata/` and serving on port `5433`.
- The chosen connection string is written to `backend/.env` as `DATABASE_URL`, which `backend/database.py` auto-loads at startup.
...
1. **Start PostgreSQL database**:
   ...
   *Database runs on `localhost:5432` with user `presence`, password `presence`, database `presence`.*
   Alternatively, point the backend at any PostgreSQL instance (including the local fallback on port `5433`) via the `DATABASE_URL` environment variable or a `backend/.env` file.
...
```powershell
# Backend: ruff (lint + import sort) and ty (type checker)
cd backend
uv run ruff check .
uv run ruff format .
uv run ty check
...
- **Connection resolution**: `DATABASE_URL` is read from the environment, then from a local `.env` file (repo root or `backend/`), defaulting to `postgresql://presence:presence@localhost:5432/presence`. If no explicit `DATABASE_URL` is set and port `5432` is unreachable, the pool automatically retries the local fallback on port `5433`.
