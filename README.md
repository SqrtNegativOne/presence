...
2. Installs and syncs backend dependencies (`uv sync --extra cloud`) and frontend packages (`bun install`)
...
## Deploy (local-only / free tier)

Presence can run **without any server-side ML runtime**: the browser computes
embeddings and the backend becomes a thin JSON + PostgreSQL service
(`fastapi`, `uvicorn`, `python-multipart`, `numpy`, `loguru`, `psycopg`).
InsightFace, ONNX Runtime, OpenCV, and Pillow live in the optional `cloud`
extra and are only installed when you ask for them.

### What runs where

| Piece | Host | Notes |
| :--- | :--- | :--- |
| Frontend (static Vite SPA) | Cloudflare Pages / Vercel / Render static | Serves `public/models/`; HTTPS required for `getUserMedia`. |
| Backend (FastAPI **lite**) | Render free Docker Web Service | Long-running process, existing `psycopg` pool works unchanged. |
| PostgreSQL | Neon free (or any managed Postgres) | Reachable from Render. |
| Local **full** stack (InsightFace) | Your machine | Unchanged `docker compose up --build`, optionally pointing at the same Neon DB. |

### Backend → Render

1. Push the repo and create a Blueprint from [`render.yaml`](render.yaml) (or a
   Docker Web Service with `dockerfilePath: ./backend/Dockerfile`).
2. Set `DATABASE_URL` to your Neon pooled connection string (it already includes
   `sslmode=require`) and `ALLOWED_ORIGINS` to your frontend URL(s).
3. `CLOUD_MODE_ENABLED=false` is set by the Blueprint. The default build installs
   the lite image only.

> Free Render services cold-start in ~30–60 s after 15 minutes idle. Render's free
> **Postgres** expires, which is why Neon is recommended for the database.

### Frontend → Cloudflare Pages (recommended)

- Root: `frontend`, build command `bun install --frozen-lockfile && bun run build`,
  output directory `frontend/dist`.
- Environment variables (build-time — changing them requires a redeploy):
  - `VITE_API_BASE_URL=https://<your-render-api>.onrender.com`
  - `VITE_ALLOW_CLOUD=false`

Vercel works the same way (framework: Vite, root `frontend`), but its Hobby plan
is non-commercial-use only — prefer Cloudflare Pages for institutional use.

### Switching modes

| Setting | `true` / unset (default) | `false` |
| :--- | :--- | :--- |
| `CLOUD_MODE_ENABLED` (backend) | `/api/students/enroll` + `/api/attendance/process` active | those two routes return **503**; local endpoints stay up |
| `VITE_ALLOW_CLOUD` (frontend) | model toggle visible | cloud option hidden, engine forced to **Local** |

Unset `VITE_API_BASE_URL` to use relative `/api` URLs (dev proxy / same-origin).
Copy [`backend/.env.example`](backend/.env.example) to `backend/.env` for local overrides.
...
│   ├── config.py                   # Env-driven config (CLOUD_MODE_ENABLED, ALLOWED_ORIGINS)
...
│   ├── services/
│   │   ├── matching.py             # Pure-numpy embedding matcher + threshold (no heavy deps)
...
- For Cloud mode, the cosine similarity threshold is `0.4`. For Local mode, it is `0.6`. These are defined in `backend/services/matching.py`.
